...
export default function App() {
  ...
  return <main className={'app-shell '+(darkMode?'theme-dark':'')}>
    ...
    <header className="top-bar" aria-label={t(language,'mapTools')}>
      <button className="icon-button search-button" aria-label={t(language,'search')} onClick={()=>setShowSearch(v=>!v)}>⌕</button><button className="icon-button" aria-label={t(language,'cameraGuide')} onClick={()=>setShowCamera(true)}>📷</button>
    ...
    <div className="walk-pill language-pill" aria-label={t(language,'guideLanguage')}><button className={language==='en'?'active':''} onClick={()=>switchLanguage('en')}>English</button><button className={language==='es'?'active':''} onClick={()=>switchLanguage('es')}>Español</button></div>
    <nav className="demo-switcher" aria-label={t(language,'demoDestinations')}>
    ...
    <aside className="map-controls">
      <button className={`round-button ${audioMode?'active-control':''}`} aria-label={t(language,'audioMode')} onClick={()=>setAudioMode(v=>!v)}>🎧</button><button className="round-button" aria-label={t(language,'chooseVoice')} onClick={()=>setShowVoiceMenu(v=>!v)}>🗣️</button>
    </aside>
    ...
    <AnimatePresence>{showNearby && <motion.section className="nearby-alert" initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} exit={{opacity:0,y:18}}><button className="nearby-dismiss" onClick={()=>setDismissedNearbyId(nearbyPOI.id)}>×</button><span className="nearby-icon">{nearbyPOI.emoji}</span><div><small>{t(language,'nearby')}</small><strong>{nearbyPOI.name}</strong><p>{audioMode?(language==='es'?'El modo de audio narrará esta parada.':'Audio mode will narrate this stop.') : t(language,'hearStory')}</p></div><button className="nearby-open" onClick={()=>setSelectedPOI(nearbyPOI)}>{t(language,'open')}</button></motion.section>}</AnimatePresence>
    {!selectedPOI && !showNearby && !showChat && !showTour && !showExplore && !showCamera && <section className="ai-dock">
      ...
      <div className={`quick-more-menu ${showMoreMenu?'open':''}`}>
        ...
        <button onClick={()=>setDarkMode(v=>!v)}>◐ {t(language,'darkMode')}: {darkMode?t(language,'yes'):t(language,'no')}</button><button onClick={()=>setKidMode(v=>!v)}>🧒 {t(language,'kidMode')}: {kidMode?t(language,'yes'):t(language,'no')}</button><button onClick={()=>{setShowCamera(true);setShowMoreMenu(false)}}>📷 {t(language,'cameraAI')}</button>
      </div>
    </section>}
    ...
    <div className={`app-signature ${(selectedPOI||showChat||showTour||showExplore||showCamera||showPractical)?'panel-open':''}`}><span>Smart AI Travel by Franklim Herwing</span><span>v0.13.1</span></div>
