...
export default function App() {
  ...
  const tourStops = useMemo(() => {
    ...
    while (remaining.length && ordered.length < 4) {
      ...
      remaining.sort((a,b) =>
        Math.hypot(a.lat-last.lat,a.lng-last.lng) - Math.hypot(b.lat-last.lat,b.lng-last.lng)
      );
  ...
  useEffect(() => {
    ...
    playNaturalNarration(narrationText(nearbyPOI), () => undefined)
      .catch(() => browserFallback(narrationText(nearbyPOI)));
  ...
  return (
    <main className="app-shell">
      ...
      <AnimatePresence>
        {showSearch && <SearchPanel pois={activePOIs} query={searchQuery} onQuery={setSearchQuery}
          onClose={()=>setShowSearch(false)} onSelect={poi=>{setSelectedPOI(poi);setShowSearch(false);}} />}
      </AnimatePresence>
      ...
      <AnimatePresence>{showNearby && <motion.section className="nearby-alert" initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} exit={{opacity:0,y:18}}>
        ...
        <div><small>YOU'RE NEARBY</small><strong>{nearbyPOI.name}</strong><p>{audioMode?'Audio mode will narrate this stop.':'Want to hear the story?'}</p></div>
        <button className="nearby-open" onClick={()=>setSelectedPOI(nearbyPOI)}>Open</button>
      </motion.section>}</AnimatePresence>

      {!selectedPOI && !showNearby && !showChat && !showTour && (
        <section className="ai-dock">
          <button className="ai-bar" onClick={()=>setShowChat(true)}>
          ...
          <div className="suggestions">
            <button onClick={()=>setShowTour(true)}>30-min tour</button>
            <button onClick={()=>{setSearchQuery('');setShowSearch(true);}}>What’s nearby?</button>
            <button onClick={()=>setShowChat(true)}>Tell me a story</button>
          </div>
        </section>
      )}

      <div className="app-signature"><span>Smart AI Travel by Franklim Herwing</span><span>v0.4.0</span></div>

      <AnimatePresence>
        {selectedPOI && !showChat && <POIBottomSheet
          poi={selectedPOI}
          position={position}
          nextPOI={nextPOI}
      ...
      <AnimatePresence>
        {showChat && <GuideChat
          context={{
            ...
            poiSummary:selectedPOI?.longDescription,
      ...
      <AnimatePresence>
        {showTour && <TourOverview
          stops={tourStops}
          onClose={()=>setShowTour(false)}
          onStart={()=>{setShowTour(false); if(tourStops[0]) setSelectedPOI(tourStops[0]);}}
        />}
      </AnimatePresence>
    </main>
  );
